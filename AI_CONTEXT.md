...
- **QoL**: Waila, Waila Harvestability, JourneyMap, GraveStone Mod, Better Storage (GTNH Edition), GT6 Ore Helper, Better Questing (with Bear's Den configs), Cooking for Blockheads, Pam's HarvestCraft, Chisel, Carpenter's Blocks, ChunkPregenerator (Server-side), ServerUtilities (GTNH Fork, Server-side), Elsewhere Border (Server-side), Morpheus (Server-side), Mouse Tweaks, Inventory Tweaks, Controlling, AppleCore, NEI Addons.
